...
# ...existing code...
        fpr, tpr, threshold = roc_curve(y_true, -brca1_subset["evo2_delta_score"])  
...
def get_genome_sequence(position, genome: str, chromosome: str, window_size=8192):
    ...
    print(f"Loaded reference genome requence window (length: {len(sequence)} bases)")
...
# Thresholds: Confidence params: {'threshold': -0.0009178519, 'lof_std': 0.0015140239, 'finc_std': 0.0009016589}

    threshold = -0.0009178519
    lof_std = 0.0015140239
    finc_std = 0.0009016589

    if delta_score < threshold:
        prediction = "Linkly pathoginic"
        confidence = min(1.0, abs(delta_score - threshold) / lof_std)

    else :
        prediction = "Linkely benign"
        confidence = min(1.0, abs(delta_score - threshold)/ finc_std)
...
class Evo2Modal:
    ...
    def analyze_single_variant(self, variant_position: int, alternative:str, genome:str, chromosome:str):
        ...
        print(f"Fetch henome sequence window, first 100: {window_seq[:100]}")
        ...
        if relative_pos < 0 or relative_pos >= len(window_seq):
             raise ValueError(f"Variant position {variant_position} is outside the fetch window (start={seq_start+1}, end={seq_start+len(window_seq)})")
